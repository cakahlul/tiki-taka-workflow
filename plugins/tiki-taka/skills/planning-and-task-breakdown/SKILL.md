...
- **If a Figma design was provided** (URL/node link), read it as part of the spec. Fetch the
  frames with `mcp__figma__get_figma_data` (see the Figma workflow in the `frontend-ui-engineering`
  skill) and inventory the screens/components/states it defines — this shapes the UI task slices
...
- [ ] [Specific, testable condition]
- [ ] [UI tasks with a design: matches Figma node `<node-id>` — layout, type, color, states — via design-system tokens]
...
**Design reference:** [Figma node link for UI tasks, or "None"]
