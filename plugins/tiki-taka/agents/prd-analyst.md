...
1. You MUST ask via the `AskUserQuestion` tool first where the PRD is read (a wiki/doc tool such as Confluence or Notion — which space/page — or a manual file) BEFORE reading anything, EXCEPT when the user/main thread has already given the location (URL/space/page/path) — if so, use it directly, do not ask again. Never assume or infer the PRD location yourself if it has not been given. Once answered: if it is in a wiki/doc tool, read it using whatever tool for it is available (its MCP server, CLI, or API); if a manual file, read that file directly. If it is in a tool that is not connected in this session, do not silently give up — tell the user it is not connected and ask them to provide the PRD as a file/paste instead.
