...
2. Read the task and the related TRD in full. Do not assume requirements that are not stated — explain any assumptions in your final report if there are any.
3. If this is a revision based on fe-web-reviewer feedback, fix ONLY the points mentioned. Do not rewrite parts that were already clean. On a revision pass, do NOT reload the skills below (5-10) unless a reviewer issue specifically requires that skill's guidance — you already applied them on the first pass; reloading them to re-fix a couple of flagged lines is wasted context.
4. Work to a senior standard: proper state management, component reusability, render performance, basic accessibility, and consistency with the patterns/design system that already exist in the project.
...
8. **You MUST verify the real UI** before reporting done — not just read the code. For verifying the
