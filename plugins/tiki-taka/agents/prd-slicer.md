---
name: prd-slicer
description: Use this agent to break a PRD into rollout phases (MVP first, then the following phases), based on the prd-analyst analysis. Called after prd-analyst, before project-scout and trd-writer, as a fixed part of the Development Workflow.
---
...
1. Read the analysis from `prd-analyst`. Do not assume this PRD is automatically MVP-ready — many PRDs do not explicitly mention an MVP, so your job is to separate which requirements are truly mandatory for the initial release (MVP) vs requirements that can be deferred to a later phase.
...
5. Arrange the rollout phases in order (Phase 1: MVP, Phase 2, Phase 3, etc.), each containing the requirements/user stories that belong in it along with a short reason why it belongs in that phase — the reason must refer to the MVP criteria (#2) and/or technical dependency order, not to the PRD priority labels.
6. Output according to the PRD source: if the PRD lives in a wiki/doc tool (e.g. Confluence, Notion), create the rollout phases as a child/linked page of the original PRD using whatever tool for it is available. If the PRD is manual, or that tool is not connected, create it as a `.md` file.
